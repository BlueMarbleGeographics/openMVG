...
// Code taken from software\UI\SfM\control_points_registration\MainWindow.cpp - 
static bool
	...
	)
{
	...
	for (auto iterCP = sfm_data.control_points.begin(); iterCP != sfm_data.control_points.end(); ++iterCP)
	{
        ...
        std::vector<Vec3> bearing; // landmark bearings for each camera
        std::vector<Mat34> poses; // camera poses
		...
		for (auto itObs = obs.begin(); itObs != obs.end(); ++itObs)
		{
            ...
            const Vec2 pt = itObs->second.x;
            bearing.emplace_back((*cam)(cam->get_ud_pixel(pt)));
            poses.emplace_back(pose.asMatrix());
		}

		// Compute the 3D point if we had enough cameras pointing at it
		if (bearing.size() >= 2)
		{
            ...
            {
                std::cerr << "Invalid triangulation for control point " << iterCP->first << std::endl;
                continue;
            }
            ...
            for (const auto & obs_it : obs)
            {
              ...
              bChierality &= cameras::CheiralityTest(bearing[i], pose, X);
              const openMVG::cameras::IntrinsicBase * cam = sfm_data.GetIntrinsics().at(view->id_intrinsic).get();
            ...
            if (bChierality) // Keep the point only if it has a positive depth
            {
              ...
              vec_triangulation_errors[iterCP->first] = reprojection_error_sum/(double)bearing.size();
            }
            else
            {
              std::cout << "Control Point " << iterCP->first << " cannot be triangulated (not in front of the cameras)" << std::endl;
              continue;
            }
		}
		else
...
int main(int argc, char **argv)
{
	enum
	{
		...
		LANDMARKS_ONLY = 3,			// only apply landmarks
	};
  ...
  unsigned int landmarks_usage = LANDMARKS_APPLY_LAST;
  ...
  catch (const std::string& s)
  {
    std::cerr
	  ...
	  << "\t0 => don't use landmarks\n"
	  << "\t1 => apply landmarks first, then GPS\n"
	  << "\t2 (default) => apply GPS, then landmarks\n"
	  << "\t3 => only apply landmarks, no GPS\n" 
	  << std::endl;
  ...
  // Apply control points
  if (!sfm_data.control_points.empty() && ( (LANDMARKS_APPLY_LAST == landmarks_usage) || (LANDMARKS_ONLY==landmarks_usage) ) )
  {
	  if (!registerProjectUsingControlPoints(sfm_data, true))
	  {
		  if (LANDMARKS_ONLY == landmarks_usage)
		  {
			  return (EXIT_FAILURE);
